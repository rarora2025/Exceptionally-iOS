...
export type ChatThread = { id: string; title: string; messages: { role: 'me' | 'ai'; text: string }[] };

// Result of the day-to-day / environments "pulls" interview.
export type PullsSaved = {
  pulls: { title: string; why: string; love: string[] }[];
  dislikes: { title: string; note: string }[];
  saved: boolean; // false while it is a draft awaiting the user's confirm
};

// A fascination artifact generated by the self-interview, shown on the profile.
export type SavedFasc = {
  interest: string;
  lens: string; // which bucket it came from (domains | work | places)
  title: string;
  oneLiner: string;
  whyItPulls: string;
  evidence: string[];
